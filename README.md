...
- 个人页新增邀请码展示、优惠券兑换和提现申请。
- 注册请求支持邀请码归因。
- 继续保持 H5 与 Dashboard 共用后端钱包、工坊、操练场和 workflow 能力。
- 钱包页新增充值渠道选择，默认微信支付，支持支付宝和手动登记。
- Workflow 发布支持开源/闭源、自由定价和单文件导入导出；创意工坊补齐排序、预估、评分、收藏、评论和举报。
...
- No-code workflow：移动端表单化配置链路、纵向节点预览、保存草稿、校验、运行、发布、开源/闭源授权、自由定价、导入和导出。
- 操练场：服务端保存对话 session，支持文生文、图生文、文生图、文生视频、图生视频。
- 创意工坊：公开样例浏览、热门/最新/收入/评分排序、费用预估、单次支付订单、运行、克隆、评分、收藏、评论和举报。
- 调用记录：任务列表、状态筛选、任务详情、节点级明细和事件查看。
- 钱包账单：微信/支付宝/手动登记充值订单、余额流水、模型扣费记录。
- 模型测试台：模型能力列表和测试调用。
...
- H5 导出优先下载 `.seeflow` 文件，小程序环境不支持文件保存时回退复制 manifest 文本。
- H5 可直接选择 `.json` / `.seeflow` 文件导入，小程序端优先使用消息文件选择能力。
- 发布到创意工坊时可选择闭源可运行或开源可克隆，并可设置售价（分）。
- 工坊详情会显示预估费用；付费运行前应先进入详情确认成本。
- 工坊支持评分、收藏、评论和举报，举报进入 Admin 审核。
